...
# https://raw.githubusercontent.com/dustmop/annotatecc65/master/annotateld65.py
...
def write_build_listing(items, outfile):
  ...
  for address,label,comment in items:
    if label and label[0:6] == '__BSS_':
      ...
      rom_start = ('%04x' % (int((int(address,16) / int(0x4000))) * 0x4000)).upper()
